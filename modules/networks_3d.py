...
class Encode3DVAE(nn.Module):
    def __init__(self, opt, out_dim, num_blocks=2):
        super(Encode3DVAE, self).__init__()
        self.opt = opt
        self.model = nn.ModuleList()
        self.model += [ConvBlock3D(opt.nc_im, opt.nfc, 3, 1, 1, 1)]  # Assuming these parameters, adjust as needed

        for i in range(1, num_blocks - 1):
            mult = 2 ** (i - 1)
            self.model += [ConvBlock3D(opt.nfc * mult, opt.nfc * mult * 2, 3, 1, 1, 1)]  # Assuming these parameters, adjust as needed

        mult = 2 ** (num_blocks - 2)
        self.model += [ConvBlock3D(opt.nfc * mult, out_dim, 3, 1, 1, 1)]  # Updated line with out_dim
...
class WDiscriminator3D(nn.Module):
  ...
  def __init__(self, opt):
    ...
    # Input channels adjusted based on generator output
    self.nfc_multi = opt.nfc_multi
    self.ker_size = opt.ker_size

    self.main = nn.Sequential(
        # 3D conv layers adjusted for full frame shape 
        nn.Conv3d(self.nfc_multi, 64, self.ker_size, stride=1, padding=1),
...
class GeneratorCSG(nn.Module):
  ...
  def __init__(self, opt):
    ...
    # Input size adjusted based on encoder output
    self.nz = opt.nz  

    self.fc = nn.Linear(self.nz, 1024*4*2*2)
    self.main = nn.Sequential(
        # 3D conv layers adjusted based on sparse frame shape
